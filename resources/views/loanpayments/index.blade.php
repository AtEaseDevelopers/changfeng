                 ...
                 <div class="col-lg-12">
                     <div class="card">
                         <div class="card-header">
                             ...
                             Loan Payments
                             <a class="pull-right" href="{{ route('loanpayments.create') }}"><i class="fa fa-plus-square fa-lg"></i></a>
...
@push('scripts')
    <script>
        ...
        function masssave(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/loanpayments/masssave",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        $(document).on("click", "#massdelete", function(e){
            ...
            if(window.checkboxid.length == 0){
                ...
                m = "Confirm to delete 1 row!<br><i>*Status with paid will not be delete*</i>"
            }else{
                m = "Confirm to delete " + window.checkboxid.length + " rows!<br><i>*Status with paid will not be delete*</i>"
            }
            $.confirm({
                title: 'Mass Delete',
                content: m,
        ...
        function massdelete(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/loanpayments/massdestroy",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massupdatestatus(ids,status){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/loanpayments/massupdatestatus",
                type:"POST",
                data:{
                ids: ids,
                status: status

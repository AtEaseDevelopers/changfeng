                 ...
                 <div class="col-lg-12">
                     <div class="card">
                         <div class="card-header">
                             ...
                             Advances
                             <a class="pull-right" href="{{ route('advances.create') }}"><i class="fa fa-plus-square fa-lg"></i></a>
...
@push('scripts')
    <script>
        ...
        function masssave(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/advances/masssave",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massdelete(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/advances/massdestroy",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massupdatestatus(ids,status){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/advances/massupdatestatus",
                type:"POST",
                data:{
                ids: ids,
                status: status

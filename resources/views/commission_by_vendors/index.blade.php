...
@section('content')
    <ol class="breadcrumb">
        <li class="breadcrumb-item">Commission By Vendors</li>
    </ol>
                 ...
                 <div class="col-lg-12">
                     <div class="card">
                         <div class="card-header">
                             ...
                             Commission By Vendors
                             <a class="pull-right" href="{{ route('commissionByVendors.create') }}"><i class="fa fa-plus-square fa-lg"></i></a>
                         ...
                         <div class="card-body">
                             @include('commission_by_vendors.table')
...
@push('scripts')
    <script>
        ...
        function masssave(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/commissionByVendors/masssave",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massdelete(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/commissionByVendors/massdestroy",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massupdatestatus(ids,status){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/commissionByVendors/massupdatestatus",
                type:"POST",
                data:{
                ids: ids,
                status: status

...
@section('content')
    <ol class="breadcrumb">
        <li class="breadcrumb-item">{{ __('focs.foc') }}</li>
    </ol>
                 ...
                 <div class="col-lg-12">
                     <div class="card">
                         <div class="card-header">
                             ...
                             {{ __('focs.foc') }}
                             <a class="pull-right" href="{{ route('focs.create') }}"><i class="fa fa-plus-square fa-lg"></i></a>
...
@push('scripts')
    <script>
        ...
        function masssave(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/focs/masssave",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massdelete(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/focs/massdestroy",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massupdatestatus(ids,status){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/focs/massupdatestatus",
                type:"POST",
                data:{
                ids: ids,
                status: status

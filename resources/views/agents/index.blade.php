...
@section('content')
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a class="pull-right text-success pr-2" id="massactive" href="#" alt="Mass active"><i
    ...
    <div class="modal fade" id="attachment_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
    ...
    <script>
        ...
        function add_attachment(){
            ...
            if (attachment.length == 0) {
                noti('e', 'Error', 'Attachment not selected');
                HideLoad();
                return false;
            }
            HideLoad();
            ...
            $.ajax({
                type: 'POST',
                url: "{{ config('app.url') }}/agents/addattachment",
                dataType: 'json',
                data: fd,
                contentType: false,
                processData: false,
        ...
        function get_attachment(id) {
            ...
            $.ajax({
                url: "{{ config('app.url') }}/agents/getattachment",
                type: "POST",
                data: {
                    id: id,
                    _token: "{{ csrf_token() }}"
                ...
                success: function(response) {
                    ...
                    $.each(response.data, function( index, value ) {
                        t = t + '<tr> <th scope="row">'+(index+1)+'</th><td><a href="'+url+value+'" target="__blank">show</a></td></tr>';
                    });
                    $('#attachment_table').html(t);
        ...
        function masssave(ids) {
            ShowLoad();
            $.ajax({
                url: "{{ config('app.url') }}/agents/masssave",
                type: "POST",
                data: {
                    ids: ids,
                    _token: "{{ csrf_token() }}"
                ...
                success: function(response) {
                    ...
                    toastr.success('Please find Save View ID: ' + response, 'Save Successfully', {
                        showEasing: "swing",
                        hideEasing: "linear",
                        showMethod: "fadeIn",
                        hideMethod: "fadeOut",
        ...
        function massdelete(ids) {
            ShowLoad();
            $.ajax({
                url: "{{ config('app.url') }}/agents/massdestroy",
                type: "POST",
                data: {
                    ids: ids,
                    _token: "{{ csrf_token() }}"
        ...
        function massupdatestatus(ids, status) {
            ShowLoad();
            $.ajax({
                url: "{{ config('app.url') }}/agents/massupdatestatus",
                type: "POST",
                data: {
                    ids: ids,
                    status: status,

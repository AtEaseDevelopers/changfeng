...
@section('content')
    <ol class="breadcrumb">
        <li class="breadcrumb-item">{{ __('special_prices.special_price')}}</li>
    </ol>
                 ...
                 <div class="col-lg-12">
                     <div class="card">
                         <div class="card-header">
                             ...
                             {{ __('special_prices.special_price')}}
                             <a class="pull-right" href="{{ route('specialPrices.create') }}"><i class="fa fa-plus-square fa-lg"></i></a>
...
@push('scripts')
    <script>
        ...
        function masssave(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/specialPrices/masssave",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massdelete(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/specialPrices/massdestroy",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massupdatestatus(ids,status){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/specialPrices/massupdatestatus",
                type:"POST",
                data:{
                ids: ids,
                status: status

...
@section('content')
    <ol class="breadcrumb">
        <li class="breadcrumb-item">{{ __('operations.operation')}}</li>
    </ol>
                 ...
                 <div class="col-lg-12">
                     <div class="card">
                         <div class="card-header">
                             ...
                             {{ __('operations.operation')}}
                             <a class="pull-right" href="{{ route('supervisors.create') }}"><i class="fa fa-plus-square fa-lg"></i></a>
...
@push('scripts')
    <script>
        ...
        function masssave(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/supervisors/masssave",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massdelete(ids){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/supervisors/massdestroy",
                type:"POST",
                data:{
                ids: ids
                ,_token: "{{ csrf_token() }}"
        ...
        function massupdatestatus(ids,status){
            ShowLoad();
            $.ajax({
                url: "{{config('app.url')}}/supervisors/massupdatestatus",
                type:"POST",
                data:{
                ids: ids,
                status: status

...
<div class="form-group col-sm-6">
    ...
    {!! Form::text('permitholder', null, ['class' => 'form-control','maxlength' => 255,'maxlength' => 255]) !!}
</div>
...
<div class="form-group col-sm-12">
    ...
    <a href="{{ route('compounds.index') }}" class="btn btn-secondary">Cancel</a>
</div>
...
@push('scripts')
    <script>
        ...
        $(document).ready(function () {
            $("#lorry_id").bind("change", function() {
                getLorryPermitHolder();
            });
            HideLoad();
        });
        function getLorryPermitHolder(){
            if($('#lorry_id').val() != ''){
                ShowLoad();
                $.ajax({
                    url: "{{config('app.url')}}/compounds/getLorryPermitHolder",
                    type:"POST",
                    data:{
